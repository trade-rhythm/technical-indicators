...
export default class SLOW_STOCH extends Indicator<SLOW_STOCHArgs> {
  ema: EMA;
  fast: FAST_STOCH;
  ...
  static minBars({ ema, fast }: SLOW_STOCHArgs): number {
    return Math.max(FAST_STOCH.minBars(fast), EMA.minBars(ema));
  }
